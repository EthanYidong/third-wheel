...
if [ ! -f "$CARGO_ROOT/ca/ca_certs/cert.pem" ]; then
    ...
    openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes -subj "/C=US/ST=private/L=province/O=city/CN=campbellc.dev"
    popd
fi

pushd "$CARGO_ROOT"
cargo build --example hyper_proxy
cargo run --example hyper_proxy -- -p 8080 &
